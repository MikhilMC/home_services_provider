...
class _WorkDetailsPageState extends State<WorkDetailsPage> {
  ...
  void _startWork() {
    final StartWorkBloc startWorkBloc = BlocProvider.of<StartWorkBloc>(context);

    startWorkBloc.add(StartWorkEvent.startWork(widget.serviceId));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocConsumer<StartWorkBloc, StartWorkState>(
        listener: (context, state) {
          state.whenOrNull(
            ...
            failure: (errorMessage) => AppHelper.showErrorDialogue(
              context,
              "Start work failed: $errorMessage",
            ),
          );
        },
